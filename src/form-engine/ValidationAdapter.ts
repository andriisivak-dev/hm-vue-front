...
export class ValidationAdapter {
     ...
     */
    static getRules(field: GFField): string | Record<string, unknown> {
        const rules: string[] = [];
        ...
        if (field.type === 'number' || field.type === 'calculation') {
            ...
            if (
                validation?.rangeMin !== undefined &&
                validation?.rangeMin !== null &&
                validation?.rangeMin !== ''
            ) {
                rules.push(`min_value:${validation.rangeMin}`);
            }
            if (
                validation?.rangeMax !== undefined &&
                validation?.rangeMax !== null &&
                validation?.rangeMax !== ''
            ) {
